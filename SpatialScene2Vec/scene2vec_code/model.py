...
class Scene2Vec(nn.Module):
    ...
    def forward(self,ng_list,do_full_eval=False):
        ...
        scene_embed = self.fc(self.linear(scene_embed))
        return scene_embed
...
class ConstraintModel(nn.Module):
    def __init__(self,encoder_q,encoder_k,T=0.07,m=0.999):
        ...
        self.m = m
        self._build_projector_and_predictor_mlps(dim=128,mlp_dim=4096)
    ...
    def forward(self,ng_list_q,ng_list_k):
        q1 = self.predictor(self.encoder_q(ng_list_q))
        q2 = self.predictor(self.encoder_q(ng_list_k))
        with torch.no_grad():
    ...
    def accuracy(self,output, target, topk=(1,)):
        ...
        with torch.no_grad():
            ...
            mrr = 0
            for k in range(maxk):
                mrr += correct[k].float().sum(0, keepdim=True).mul_(1/(batch_size*(k+1)))
            res = []
            res.append(mrr)
            for k in topk:
    ...
    def _build_projector_and_predictor_mlps(self, dim, mlp_dim):
        ...
        # projectors
        self.encoder_q.fc = self._build_mlp(3, hidden_dim, mlp_dim, dim)
        self.encoder_k.fc = self._build_mlp(3, hidden_dim, mlp_dim, dim)
