...
class Trainer():
    def __init__(self,poiset,centerset,ng_pair_list,feature_embedding,console=True) -> None:
        ...
        self.log_name = 'Model_' + time.strftime("%Y-%m-%d_%H_%M_%S", time.localtime())
        self.log_file = self.path + '/log/' + self.log_name + '.log'
        self.model_file = self.path+'/model_save/' + self.log_name + '.pth.tar'
        ...
        if self.cl_model is not None:
            ...
            for i in range(max_iter):
                ...
                if i%log_every == 0:
                    ...
                    self.logger.info("Iter: {:d}; Train MRR: {:f}, HIT@1: {:f}, HIT@3: {:f}, HIT@5: {:f}, HIT@10: {:f}".format(i, acc[0][0],acc[1][0]/100,acc[2][0]/100,acc[3][0]/100,acc[4][0]/100))
                    # break
                    if not self.model_file is None:
                        torch.save({'iter':i,
                                    'state_dict':self.cl_model.state_dict(),
                                    'optimizer':self.optimizer.state_dict(),
                                    'scaler':self.scaler.state_dict()}, self.path+'/checkpoints/'+self.log_name+'_%04d.pth.tar'%i)
        else:
            i = 0
    ...
    def run_batch(self,iter_count,batch_size,do_full_eval):
        ...
        with torch.cuda.amp.autocast(True):
            loss,acc = self.cl_model(ng_list_q,ng_list_k)
